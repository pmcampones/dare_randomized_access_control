...
func main() {
	...
	addBob := hashgraph.NewNode(crdt.Add(alice, bob, 20), []*hashgraph.Node{initNode})
	claire, _ := uuid.NewRandomFromReader(r)
	dillan, _ := uuid.NewRandomFromReader(r)
	addClaire := hashgraph.NewNode(crdt.Add(alice, claire, 20), []*hashgraph.Node{addBob})
	addDillan := hashgraph.NewNode(crdt.Add(bob, dillan, 20), []*hashgraph.Node{addBob})
	hashgraph.NewNode(crdt.Post(alice, "hello everyone"), []*hashgraph.Node{addClaire, addDillan})
	initNode.RunHashgraph(0)
